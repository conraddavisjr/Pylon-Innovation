...
;(function(window) {
	...
	function initEvents() {
		...
		filterItems.on('click', function() {
			...
			setTimeout(function(){
				var sliderEl = $('.slider'),
						zoomCtrl = $('.zoomer, .zoomer__area'),
						closeContentCtrl = $('button.button--close', contentEl);
				console.log('zoomCtrl Element: ', zoomCtrl)
		
				// open items
				$('.slider').on('click', function() {
					console.log('zoomCtrl Clicked!')
					openItem($("[data-content='content-1'], .zoomer"));
				});
			}, 1000);

			// close content
			closeContentCtrl.on('click', closeContent);
		});
	...
	function openItem(item) {
		...
		// disallow scroll
		scrollContainer.on('scroll', noscroll);
		// apply transforms
